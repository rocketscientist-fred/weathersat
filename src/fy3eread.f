...
c-- Pre-SatDump-0.0.38 value for nfy - modified to 0.0.38 value on 20211104
c      parameter (nchan=2, npix=6144, nrscans=40, ngrey=4096, nfy=12357)
      parameter (nchan=2, npix=6144, nrscans=40, ngrey=4096, nfy=9376)
c
      logical   swcorrect, swdebug, swzero(3), swzerorec, swinvert
      ...
      integer*4 chshift(nchan), lunout(nchan), iline_store(1000000), i_zero(3), dy2k, doy, lunoutrgb(3), chshiftextra
      integer*4 chshiftused(nchan), imir_store(1000000), imir_rec(1000000, nchan), nrlines, imir_sum, imir_act
c
      character*2 cband
      character*5 cnpix
...
c-- Process all the command line arguments and take initialisation actions - if required
      ...
      do i = 2,30
        ...
        if (index(argstring(1:lnblnk(argstring)),'invert').ne.0)  swinvert  = .true.
      enddo
...
c-- High Res records
          ...
          if (swdebug) write (*,*) j+nbyte-jref, nfy, j + nbyte, jref
...
c-- For the lines below, I suspect that for i_sync_bit 4, 5, 6, 7 will need i_sync_byte = 10 and for 0, 1, 2, 3  i_sync_byte=9
      ...
      do i = 1, nchan
        call get_lun(lun(i))
        write (cband,'(I2.2)') i
        open (unit=lun(i),file=filenm(1:lnblnk(filenm))//'-band'//cband//'.dat',access='direct',form='unformatted',recl=2*npix)
        irec(i) = 0
      enddo
        ...
        if (n_actual.gt.0) then
          do i = 1, n_actual
            ...
            if (j.eq.highres_offset(l)+nfy) then
              ...
              imir = ishft(iand(alignbuf(159),'10'x),-4)
              iline = alignbuf(13)
              if (iline.lt.0) iline = iline + 256
c-- Alan's mersi.bin files contain mysteriously empty lines which confuse my buffer flushing, so remove artificially - not relevant for FY3E ?
              swzerorec = .false.
              if (iline.eq.0) then
                swzerorec = .true.
                i1 = 13
                do while (swzerorec.and.i1.le.112)
              ...
              if (swzerorec) then
                ...
                iline_store(l) = iline
                imir_store(l)  = imir
c                if (swdebug) write (*,'(100(z2.2,1x),3I10,1x)') (alignbuf(i1), i1 = 1, 100), l, k, alignbuf(13)
                if (swdebug) write (*,'(100(z2.2,1x),2I10)') (alignbuf(i1), i1 = 140, 239), imir, alignbuf(13)
              endif
 4            l = l + 1
              if (l.gt.k) goto 3
            endif
...
c-- and then flush and clean the current buffers
                do i1 = 1, nchan
                  ...
                  enddo
                  imir_act = nint(float(imir_sum)/float(nrlines)) + 1
                  if (imir_act.lt.1.or.imir_act.gt.2) imir_act = 1
                  do i2 = 1, 40
                    irec(i1) = irec(i1) + 1
                    imir_rec(irec(i1), i1) = imir_act
                    call write_buffer(lun(i1), irec(i1), channel_image(1,i2,i1), npix)
c                    if (swcorrect) call hist_create(channel_image(1,i2,i1), npix, i2, i1, imir_act)
                    do i3 = 1, npix
...
c-- Convert data files to images
        do i = 1, nchan
          write (cband,'(I2.2)') i
          if (swinvert) then
            write (command,'(''convert -depth 16 -endian lsb -size '',A5,I5.5,'' gray:'',a,'' -equalize -depth 16 -rotate 180 '',a)') cnpix, irec(i), 
     *              filenm(1:lnblnk(filenm))//'-band'//cband//'.dat', 
     *              filenm(1:lnblnk(filenm))//'-band'//cband//'.png'
          else
            write (command,'(''convert -depth 16 -endian lsb -size '',A5,I5.5,'' gray:'',a,'' -equalize -depth 16 -rotate 180 -negate '',a)') cnpix, irec(i), 
     *              filenm(1:lnblnk(filenm))//'-band'//cband//'.dat', 
     *              filenm(1:lnblnk(filenm))//'-band'//cband//'.png'
          endif
      ...
      do i = 1, nchan
        ...
        write (cband,'(I2.2)') i
        open (unit=lunout(i),file=filenm(1:lnblnk(filenm))//'-band'//cband//'-shift.dat',access='direct',form='unformatted',recl=2*npix)
      enddo
      do i = 1, nchan
        do j = 1, irec(i)
          read (lun(i),rec=j) pixbuf
          if (i.eq.1) then
            if (mod(j,2).eq.0) then
              chshiftused(i) = chshift(i) + chshiftextra
            else
              chshiftused(i) = chshift(i)
            endif
          else
            if (mod(j,2).eq.0) then
              chshiftused(i) = chshift(i) + chshiftextra
...
c          if (chshift(i).ne.0) then
            if (chshiftused(i).gt.0) then
              do k = npix - chshiftused(i), 1, -1
                pixbuf(k+chshiftused(i)) = pixbuf(k)
              enddo
              do k = 1, chshiftused(i)
                pixbuf(k) = 0
              enddo
            else
              do k = abs(chshiftused(i)) + 1, npix
                pixbuf(k+chshiftused(i)) = pixbuf(k)
              enddo
              do k = npix + chshiftused(i) + 1, npix
                pixbuf(k) = 0
              enddo
            endif
c          endif
...
c-- Create a false colour composite from the shifted dat file for ch 3, 2 and 1 - in two different ways
     ...
     *              filenm(1:lnblnk(filenm))//'-RGB-combine.png',' alt=833.0 sharpen bowtie=mersi1 theta1=0.0 theta2=1.2 bowgamma=1.0 debug'
      else
        write (command,'(''./deproject.exe '',a,a)') 
     *              filenm(1:lnblnk(filenm))//'-RGB-combine.png',' alt=833.0 sharpen bowtie=mersi1 theta1=0.0 theta2=1.2 bowgamma=1.0'
      endif
      write (*,*) command(1:lnblnk(command))
      call my_system(command(1:lnblnk(command)))
c
...
c
      ...
      subroutine find_fy_mask_short(buffer, n, nbyte, nbit)
      implicit none
      integer*4 n, nbyte, nbit
      integer*1 buffer(n), nextbuf(n)
c
...
c
      ...
      xr(1) = 0.
      xr(2) = float(nxin)
c      xr(1) = float(int(i1/100)       * 100)
c      xr(2) = float((int(i2/100) + 1) * 100)
      yr(1) = hist(2,1)
      yr(2) = yr(1)
      do i = 2, nxin - 1
        do j = 1, nyin
