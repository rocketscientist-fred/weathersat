      ...
      parameter (nchan=6, npix=8192, nrscans=40, ngrey=4096, nfy=12357)
c
      ...
      integer*4 chshift(nchan), lunout(nchan), iline_store(1000000), i_zero(3), chshiftextra
...
c-- Fix for Alan's mersi.bin files - my line 0 problem
          if (j+nbyte-jref.eq.191*nfy) then
            k = k + 1
            highres_offset(k) = j + nbyte - nfy
          endif
...
c-- So where the sync marker aligns (nbit) is 1 bit below what value to use :-)
c
      i_sync_bit = i2 - 1
        ...
        if (n_actual.gt.0) then
          do i = 1, n_actual
            ...
            if (j.eq.highres_offset(l)+nfy) then
              ...
              imir  = ishft(iand(alignbuf(68),'F0'x),-4) + 1
...
c              if (iline.eq.1.and.imir.eq.2) goto 2
              call bit_align_buffer(highres, nfy, alignbuf, nfy, i_sync_bit) 
              i_channel = ((iline - 1) / 40) + 1
              if (i_channel.gt.nchan) goto 2
              i_off = 61 + i_sync_byte
              do i2 = 1, 8189, 2
                i1 = i_off + ((i2 - 1) / 2) * 3
...
c
c-- Check if there is a global wrap around. If so flush empty buffers to keep the 6 channel images sync'd
...
c-- and then flush and clean the current buffers
                do i1 = 1, nchan
                  ...
                  if (imir_act.lt.1.or.imir_act.gt.2) then
                    nrlines  = 0
                    imir_sum = 0
                    do i2 = 1, 40
                      if (imir_buf(i2, i1).ne.-1) then
...
c-- Final sync align
      ...
      enddo
c-- If needed, temporarily fix Alan's "Line 0 problem" here
      if (swfix) then
        write (cband,'(I2.2)') 1
...
c-- Analyse the offset correction and add it to the channels
      if (swcorrect) then
        do i = 1, nchan
          do j = 1, irec(i)
...
c-- Convert data files to images
     ...
     *            filenm(1:lnblnk(filenm))//'-band'//cband//'.png'
      ...
      enddo
c-- In FY3D bands 5 and 6 have the same interline shift feature that FY3E shows. Fixed here.
      do i = 1, nchan
        do j = 1, irec(i)
          read (lun(i),rec=j) pixbuf
          if (i.eq.5) then
...
c-- Create a false colour composite from the shifted dat file for ch 3, 2 and 1 - in two different ways
     ...
     *            filenm(1:lnblnk(filenm))//'-RGB-combine.png',' alt=833.0 sharpen bowtie=mersi1'
      write (*,*) command(1:lnblnk(command))
      call my_system(command(1:lnblnk(command)))
c
      stop
...
c            if(i.eq.1.and.j.eq.40) goto 2
            if (nhist(j, i, l).gt.0) then
              ...
              enddo
              do i1 = i_histmax, 2, -1
c-- FY3D band dependent limits (ad-hoc)
                if (i.le.4) then
                  if (hist(i1, j, i, l).le.nint(0.30*histmax).and.hist(i1, j, i, l).gt.0) goto 1
                else
                  if (hist(i1, j, i, l).le.nint(0.55*histmax).and.hist(i1, j, i, l).gt.0) goto 1
                endif
              enddo
 1            continue
              i2 = i1 + 10
...
c-- For the time being only plot the spectra here
            if (((j.ge.2.and.l.eq.1).or.(j.ge.1.and.l.eq.2)).and.swdebug) then
...
c
      ...
      xr(1) = 0.
      xr(2) = float(nxin)
c      xr(1) = float((int(i1/100) - 1) * 100)
c      xr(2) = float((int(i2/100) + 2) * 100)
      yr(1) = hist(2,1)
      yr(2) = yr(1)
      do i = 2, nxin - 1
        do j = 1, nyin
